...
function initTopbar() {
    ...
    const supervisorMenuBtn = document.getElementById("supervisorMenuBtn");
    const supervisorDropdown = document.getElementById("supervisorDropdown");

    if (notifBtn && notifDropdown) {
        notifBtn.addEventListener("click", (e) => {
            e.stopPropagation();
            if (supervisorDropdown) supervisorDropdown.style.display = "none";
            notifDropdown.classList.toggle("active");
        });
    }

    if (supervisorMenuBtn && supervisorDropdown) {
        supervisorMenuBtn.addEventListener("click", (e) => {
            e.stopPropagation();
            if (notifDropdown) notifDropdown.classList.remove("active");

            supervisorDropdown.style.display =
                supervisorDropdown.style.display === "block" ? "none" : "block";
        });
    }
    ...
    if (supervisorDropdown) {
        supervisorDropdown.addEventListener("click", (e) => e.stopPropagation());
    }
}
...
function initActiveLinks() {
    const links = document.querySelectorAll(
        ".supervisor-pages-container a, .student-pages-container a"
    );
...
function initDashboardStats() {
    ...
    if (statPending) {
        statPending.textContent = reports.filter(r => r.status.includes("بانتظار")).length;
    }
    if (statEvaluations) statEvaluations.textContent = "2";
    ...
    if (reportsPreview) {
        ...
        <tr>
          ...
          <td><button class="action-btn" type="button">عرض</button></td>
        </tr>
      `;
        });
    }
...
function initStudentsPage() {
    ...
    if (!studentsTable || !searchStudent || !filterStatus) return;
    ...
    function renderTable() {
        ...
        <tr>
          ...
          <td>
            <button class="action-btn student-view-btn" type="button" data-id="${s.id}">عرض</button>
          </td>
        </tr>
      `;
        });

        const viewButtons = studentsTable.querySelectorAll(".student-view-btn");
        viewButtons.forEach((btn) => {
            btn.addEventListener("click", () => {
                ...
                if (modalStudentName) modalStudentName.textContent = student.name;
                if (modalStudentId) modalStudentId.textContent = student.id;
                if (modalStudentCompany) modalStudentCompany.textContent = student.company;
                if (modalStudentStatus) modalStudentStatus.textContent = student.status;
                if (modalStudentLastReport) modalStudentLastReport.textContent = student.lastReport;
...
function initReportsPage() {
    ...
    if (!reportsTable || !searchReport || !filterReportStatus) return;
    ...
    function render() {
        ...
        <tr>
          ...
          <td><button class="action-btn report-view-btn" type="button" data-id="${r.id}">عرض</button></td>
        </tr>
      `;
        });

        const viewButtons = reportsTable.querySelectorAll(".report-view-btn");
        viewButtons.forEach((btn) => {
            btn.addEventListener("click", () => {
                ...
                if (reportTitle) reportTitle.textContent = `📄 تقرير ${rep.week}`;
                if (reportStudent) reportStudent.textContent = rep.student;
                if (reportWeek) reportWeek.textContent = rep.week;
                if (reportContent) reportContent.textContent = rep.content;
...
========================= */
function initAdminStudentsManager() {
    ...
    if (
        !tbody || !searchInput || !statusFilter || !companyFilter ||
        !stTotal || !stActive || !stPaused || !stPending ||
        !shownCount || !allCount ||
        !addBtn || !seedBtn || !clearBtn ||
        !dialog || !dlgClose || !cancelBtn || !saveBtn ||
        !dlgTitle || !dlgHint ||
        !studentId || !fullName || !uniId || !company || !status || !notes
    ) {
        return;
    }
    ...
    saveBtn.addEventListener("click", () => {
        ...
        if (!nameVal || !uniVal || !compVal || !stVal) {
            alert("من فضلك عبّي الحقول الأساسية.");
            return;
        }
        ...
        const conflict = students.find(
            (s) => s.uniId === uniVal && String(s.id) !== String(idVal || "")
        );

        if (conflict) {
            alert("الرقم الجامعي موجود مسبقًا.");
            return;
        }
    ...
    function refreshCompanyFilter() {
        ...
        const companies = Array.from(new Set(students.map((s) => s.company)))
            .filter(Boolean)
            .sort();
    ...
    //       <div class="actions">
    //         <button class="icon-btn primary" type="button" data-action="edit" data-id="${s.id}">✏️ تعديل</button>
    //         <button class="icon-btn" type="button" data-action="toggle" data-id="${s.id}">
    //           ${s.status === "active" ? "⏸️ إيقاف" : "✅ تفعيل"}
    //         </button>
    //         <button class="icon-btn danger" type="button" data-action="delete" data-id="${s.id}">🗑 حذف</button>
    //       </div>
